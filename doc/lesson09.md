...
- Если не работает `datepicker()` проверьте - подключен ли скрипт в mealList.jsp? Можно посмотреть из браузера по Ctrl+U и кликнуть ссылку на скрипт- должен открыться `jquery.noty.packaged.min.js`.
